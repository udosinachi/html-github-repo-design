document.querySelector('.octicon-three-bars').addEventListener('click', (e) => {
  if (
    document
      ...
      .classList.contains('show-mobile-nav-list')
  ) {
    document
      .querySelector('.search_wrapper')
      .classList.remove('show-mobile-nav-list')
  } else {
    document
      .querySelector('.search_wrapper')
      .classList.add('show-mobile-nav-list')
  }
  if (
    document
      .querySelector('.nav_menu_wrapper')
      .classList.contains('show-mobile-nav-list')
  ) {
    document
      .querySelector('.nav_menu_wrapper')
...
fetch(baseUrl, {
  ...
  .then((res) => {
    ...
    document.querySelector('.totalCount').textContent =
      res.data.viewer.repositories.totalCount
    ...
    let count = 0
    let publicReposCount = res.data.viewer.repositories.nodes.filter(
      (el) => !el.isPrivate
    ).length
    ...
    res.data.viewer.repositories.nodes.forEach((el) => {
      ...
      let child = `
                ...
                <div class="repo_info">
                    ...
                    <div class="repo_info_content">
                        <div class="content_info">
                            <span class="repo-language-color" style="background: ${
                              el.primaryLanguage.color
                            }"></span><span> ${el.primaryLanguage.name}</span>
                        </div>
        ...
        `
      document
        .querySelector('.repo_list_container')
